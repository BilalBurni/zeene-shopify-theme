...
function collectCartSections() {
  ...
  for (const el of document.querySelectorAll(DAWN_CART_TAGS.join(','))) {
    ...
    for (const entry of entries ?? []) {
      ...
      // getSectionsToRender() has two shapes: cart-items (entry.section is the parent, entry.selector a child node) and cart-drawer / cart-notification (entry.id IS the mount, no parent wrapper).
